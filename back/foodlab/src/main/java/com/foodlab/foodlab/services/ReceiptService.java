...
////
////        for (int i = 0; i < 5; i++) {
////            op.add(new OrdenProducto(Optional.ofNullable(prods.get(i)), counter, ""));
////            counter++;
////        }
////        return op;
////    }
...
//    }
    ...
    public Receipt findByOrderId(Integer idOrder) {
        return receiptRepo.findByOrderIdOrder(idOrder);
    }

    public final double calculateDiscount(int cantidad) {
        if (cantidad > 4) {
            return 0.05;
        }
        return 0;
    ...
    public final double calculateTotal(Receipt factura, Order order) {
        return factura.getDiscount() == 0 ? (order.getTotal() + factura.getDelivery())
                : ((order.getTotal() - (order.getTotal() * factura.getDiscount()) + factura.getDelivery()));
    }
    ...
    public byte[] generateReceiptPdf(Receipt receipt) {
        ...
        try {
            ...
            document.add(p4);
            Paragraph p5 = new Paragraph("Método de Pago: " + receipt.getOrder().getUser().getMetodoPago().getTipoTarjeta());
            p5.setAlignment(Element.ALIGN_CENTER);
            document.add(p5);

            String numCard = String.valueOf(receipt.getOrder().getUser().getMetodoPago().getNumeroTarjeta());
            Paragraph p6 = new Paragraph("Últimos 4 digitos: " + numCard.substring((numCard.length() - 4), numCard.length()));
            p6.setAlignment(Element.ALIGN_CENTER);
            document.add(p6);
            Paragraph p7 = new Paragraph("Tipo: " + receipt.getOrder().getUser().getMetodoPago().getFranquicia());
            ...
            for (OrdenProducto prod : receipt.getOrder().getProducts()) {
                Paragraph p9 = new Paragraph("    - " + prod.getProducto().getNombre() + "  $" + prod.getProducto().getPrecio());
